...
function init() {
  ...
  // Assignment 4: Add textures
  var texture = new THREE.TextureLoader().load("textures/crawfis.jpg");
  ...
  for ( i = 0; i < segments; i++ ) {
    for ( j = 0; j < segments; j++ ) {
      ...
      var d = i * ( segments + 1 ) + ( j + 1 );
      var c = i * ( segments + 1 ) + j;
      var b = ( i + 1 ) * ( segments + 1 ) + j;
      var a = ( i + 1 ) * ( segments + 1 ) + ( j + 1 );
      // generate two faces (triangles) per iteration
